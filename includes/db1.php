 ...
 */
/**
 * Get products with filtering and pagination - SIMPLIFIED & RELIABLE VERSION
 */
function getProducts($filters = [], $order_by = 'created_at DESC', $page = 1, $per_page = PRODUCTS_PER_PAGE)
{
    ...
    if (!empty($filters['search'])) {
        $where_conditions[] = '(p.name LIKE :search OR p.description LIKE :search OR p.tags LIKE :search OR p.short_description LIKE :search)';
    ...
    // Color Filter - ADDED THIS SECTION
    if (!empty($filters['colors'])) {
        $where_conditions[] = 'FIND_IN_SET(:color, p.available_colors)';
        $params['color'] = $filters['colors'][0]; // Using first color if array
    ...
    // ===============================================
    // SORTING LOGIC - FIXED RATING SORT
    // ===============================================
    ...
    // Sort mapping including rating
    $sort_mapping = [
        'created_at' => 'p.created_at',
        'price'      => '(CASE WHEN p.sale_price IS NOT NULL THEN p.sale_price ELSE p.price END)',
        'name'       => 'p.name',
        'featured'   => 'p.featured',
        'rating'     => 'average_rating' // This will be handled later
    ];
    ...
    if ($sort_field === 'rating') {
        $order_clause = "ORDER BY average_rating {$sort_direction}";
    } else {
