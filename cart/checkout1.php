...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'process_order' && empty($errors)) {
        ...
        // Check product availability and stock for COD orders
        if (empty($errors) && $payment_method === 'cod') {
            foreach ($cart_items as $item) {
                ...
                if (!$product || $product['status'] !== 'active') {
                    $errors[] = 'The product "' . $item['name'] . '" is not available.';
                } elseif ($product['stock_quantity'] < $item['quantity']) {
                    $errors[] = 'Insufficient stock for "' . $item['name'] . '". Available: ' . $product['stock_quantity'];
                }
            }
        }
